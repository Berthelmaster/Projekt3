...
void T1_webApp::run()
{
  ...
  while(true)
  {
    char coffee, water, strength;
    std::cout << "Angiv type kaffe: " << '\n';
    std::cin >> coffee;
    std::cout << "Angiv kaffestørrelse: " << '\n';
    std::cin >> water;
    std::cout << "Angiv kaffesstyrke: " << '\n';
    std::cin >> strength;
    sendCoffeeOrder(water, coffee, strength);
...
void T1_webApp::handler(osapi::Message* msg, unsigned long id)
{
  switch(id)
  {
    case(ID_STATUS_IND):
    {
      ...
      //opdater status
      std::cout << "kaffesstatus: "<<ind->coffeeStatus_ << '\n';
    }
    break;
  }
}
